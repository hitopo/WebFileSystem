...
var swfobject = function() {
                ...
                if (ah.ie && ah.win) {
                    ...
                    if (af == top) {
                        (function() {
                            ...
                            try { aL.documentElement.doScroll("left"); } catch (a) {
    ...
    function aw(a) {
        ...
        if (b && b.nodeName == "OBJECT") {
            if (ah.ie && ah.win) {
                ...
                (function() {
                    if (b.readyState == 4) { aT(a); } else { setTimeout(arguments.callee, 10); }
                })();
    ...
    function az(b, f, a, c) {
        ...
        if (ah.ie && ah.win) {
            if (aH && typeof aH.addRule == aD) { aH.addRule(b, f); }
        } else {
...
SWFUpload.prototype.initSettings = function() {
    this.ensureDefault = function(c, d) { this.settings[c] = (this.settings[c] == undefined) ? d : this.settings[c]; };
...
SWFUpload.prototype.destroy = function() {
    try {
        ...
        if (g && typeof(g.CallFunction) === "unknown") {
            for (var j in g) {
                try {
                    if (typeof(g[j]) === "function") { g[j] = null; }
                } catch (h) {}
            }
...
SWFUpload.prototype.queueEvent = function(d, f) {
    ...
    if (typeof this.settings[d] === "function") {
        this.eventQueue.push(function() { this.settings[d].apply(this, f); });
        setTimeout(function() { e.executeNextEvent(); }, 0);
    } else {
...
SWFUpload.prototype.cleanUp = function(f) {
    try {
        if (this.movieElement && typeof(f.CallFunction) === "unknown") {
            ...
            for (var h in f) {
                try {
                    if (typeof(f[h]) === "function") { f[h] = null; }
                } catch (e) {}
            }
        }
    } catch (g) {}
...
SWFUpload.Console.writeLine = function(g) {
    ...
    } catch (h) { alert("Exception: " + h.name + " Message: " + h.message); }
};
(function(c) {
    var b = {
        init: function(d, e) {
            return this.each(function() {
                ...
                if (h) {
                    ...
                    i.button = f;
                    i.queue = k;
                    if (j.onInit) { j.onInit.call(n, i); }
                } else {
                    if (j.onFallback) { j.onFallback.call(n); }
                }
            });
        },
        cancel: function(d, f) {
            ...
            this.each(function() {
                ...
                if (e[0]) {
                    if (e[0] == "*") {
                        ...
                        if (j.onClearQueue) { j.onClearQueue.call(l, g); }
                    } else {
                        for (var m = 0; m < e.length; m++) {
                            i.cancelUpload(e[m]);
                            c("#" + e[m]).find(".data").removeClass("data").html(" - Cancelled");
                            c("#" + e[m]).find(".uploadify-progress-bar").remove();
                            c("#" + e[m]).delay(1000 + 100 * m).fadeOut(500, function() { c(this).remove(); });
                        }
                    }
                } else {
                    ...
                    $item.delay(1000).fadeOut(500, function() { c(this).remove(); });
                }
            });
        },
        destroy: function() {
        ...
        disable: function(d) {
            this.each(function() {
                ...
                if (d) {
                    ...
                    if (f.onDisable) { f.onDisable.call(this); }
                } else {
    ...
    var a = {
        ...
        onUploadComplete: function(f) {
            ...
            if (this.queueData.uploadQueue[0] == "*") {
                if (this.queueData.queueLength > 0) { this.startUpload(); } else {
                    this.queueData.uploadQueue = [];
                    if (g.onQueueComplete) { g.onQueueComplete.call(this, this.queueData); }
                }
            } else {
                if (this.queueData.uploadQueue.length > 0) { this.startUpload(this.queueData.uploadQueue.shift()); } else {
                    this.queueData.uploadQueue = [];
                    if (g.onQueueComplete) { g.onQueueComplete.call(this, this.queueData); }
                }
            }
        ...
        onUploadStart: function(d) {
            ...
            if (e.checkExisting) {
                c.ajax({
                    ...
                    data: { 
                            'filename': d.name,
                            'userId':e.formData.userId
                        },
                    success: function(h) {
                        if (h == 1) {
                            var g = confirm('A file with the name "' + d.name + '" already exists on the server.\nWould you like to replace the existing file?');
                            if (!g) {
                               $('#uploadify').uploadify('cancel',d.id);
                                c("#" + d.id).remove();
                                if ($('#uploadify').queueData.uploadQueue.length > 0 && $('#uploadify').queueData.queueLength > 0) {
                                    if ($('#uploadify').queueData.uploadQueue[0] == "*") { $('#uploadify').startUpload(); } else { $('#uploadify').startUpload($('#uploadify').queueData.uploadQueue.shift()); }
                                }
                            }
                        }
                    }
            ...
            if (e.onUploadStart) { e.onUploadStart.call($('#uploadify'), d); }
        },
    ...
    c.fn.uploadify = function(d) {
        if (b[d]) {
            ...
            } else { c.error("The method " + d + " does not exist in $.uploadify"); }
        }
    };
})($);
